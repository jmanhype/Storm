...
from groq import Groq as GroqClient  # Ensure this import points to the correct location
from dsp.modules.lm import LM
from utils import fetch_wikipedia_links, fetch_table_of_contents

# Define your custom Language Model class using Groq
class Groq(LM):
    def __init__(self, model="mixtral-8x7b-32768", max_tokens=2000, **kwargs):
    ...
    def basic_request(self, prompt, **kwargs):
        # Ensure max_tokens is handled properly without duplication
        kwargs.setdefault('max_tokens', self.max_tokens)  # Set default if not provided, avoid duplication
        response = self.client.chat.completions.create(
            messages=[{"role": "user", "content": prompt}],
            model=self.kwargs.get("model", "mixtral-8x7b-32768"),
            **kwargs  # Only one source for max_tokens now
        )
...
# Initialize DSPy settings using the Groq LM
def initialize_dspy():
